...
```{r}
data <- import("https://raw.githubusercontent.com/yehezkinovandri/mpdw/7975608dd9ec5128c86261ea6c5a6c2d183fb17f/Pertemuan%201/daily-website-visitors.csv")
...
Karena kolom Page.Loads masih bertipe character, maka perlu diubah ke tipe Numeric

```{r}
data$Page.Loads <- as.numeric(gsub(",", "", data$Page.Loads))
```

Hanya mengambil kolom Date dan Page.Loads

```{r}
data <- data %>% select(Date, Page.Loads)
```
...
```{r}
ts.plot(data.ts, xlab="Time Period ", ylab="Page Loads", 
        main = "Time Series Plot")
points(data.ts)
```

Data memiliki lonjakan-lonjakan yang dapat diidentifikasi seperti Seasonal sehingga cocok dengan Single Moving Average (SMA) dan Single Exponential Smoothing (SES)
...
Hasilnya sama, RMSE1 > RMSE2, artinya model 2 lebih baik daripada model 1 dengan rata-rata error per prediksi sekitar 852 Page Loads.

Model 1 jauh lebih buruk (error rata-rata sekitar 950).
...
Dari sudut pandang praktis, model terbaik menghasilkan error rata-rata sekitar 852 Page Loads per periode.