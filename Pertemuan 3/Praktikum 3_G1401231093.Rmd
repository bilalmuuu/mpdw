---
title: "Praktikum 3 Investigasi dan Penanganan Autokorelasi"
...
# Input Data IPM Kota Medan (2014-2025)
tahun <- 2015:2024
ipm <- c(78.87, 79.34, 79.98, 80.65, 80.97, 80.98, 81.21, 81.76, 82.19, 82.76)
...
Lihat! **Nilai Adjusted R-squared sangat tinggi (0.9661)** dan koefisien `tahun` **sangat signifikan (p-value \< 0.001)**. Jika kita berhenti di sini, kita akan menyimpulkan ini adalah model yang fantastis. Tapi seorang detektif yang baik tidak akan berhenti di permukaan.
...
Plot di atas menunjukkan pola yang **tidak acak**. Perhatikan bagaimana sisaan cenderung berkelompok: titik pertama di bawah nol, lalu naik terus, dan turun lagi. Ini adalah gejala visual yang kuat dari autokorelasi positif. Sisaan positif cenderung diikuti sisaan positif, dan sebaliknya.
...
-   P-value yang dihasilkan **sangat kecil (0.006332)**, yaitu di bawah tingkat signifikansi 0.05.
...
Pertama-tama akan dicari di mana kira-kira ρ yang menghasilkan SSE minimum. Pada hasil di atas terlihat ρminimum ketika 0.5. Namun, hasil tersebut masih kurang teliti sehingga akan dicari kembali ρ yang lebih optimum dengan ketelitian yang lebih. Jika sebelumnya jarak antar ρ yang dicari adalah 0.1, kali ini jarak antar ρ adalah 0.001 dan dilakukan pada selang 0.2 sampai dengan 0.5.
...
text(x=0.468, y=0.2503057, labels = "rho = 0.468", cex = 0.8)
```

```{r}
model_hl <- hildreth.lu(dt_ipm$ipm, dt_ipm$tahun, rho = 0.468)
summary(model_hl)
```

```{r}
#Transformasi Balik
cat("y = ", coef(model_hl)[1]/(1-0.468), "+", coef(model_hl)[2],"x", sep = "")
```
...
-   Hasil summary memberikan model akhir yang juga sudah "sehat", dengan statistik D-W yang sudah membaik dengan taraf 96% atau 0.04.
